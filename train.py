...
def train_one_epoch(model, data_loader):
    ...
    for data in data_loader:
        ...
        if torch.cuda.is_available():
            ...
            #node_features_pre = Variable(node_features_pre.cuda().float())
           ...
           # node_features_pre = Variable(node_features_pre.float())
            G_batch.edata['ex'] = Variable(G_batch.edata['ex'].float())
...
def train(model, train_dataframe, valid_dataframe, fold = 0):
    train_loader = DataLoader(dataset=ProDataset(train_dataframe), batch_size=BATCH_SIZE, shuffle=True, num_workers=1, collate_fn=graph_collate)
    valid_loader = DataLoader(dataset=ProDataset(valid_dataframe), batch_size=BATCH_SIZE, shuffle=True, num_workers=1, collate_fn=graph_collate)
...
def cross_validation(all_dataframe, fold_number=5):
    ...
    print("The base Model type 1:", BASE_MODEL_TYPE_1)
    print("The base Model type 2:", BASE_MODEL_TYPE_2)
   ...
   # print("Layer:", LAYER)
    ...
    for train_index, valid_index in kfold.split(sequence_names, sequence_labels):
        ...
        model = MGMAPPIS( INPUT_DIM, HIDDEN_DIM1,HIDDEN_DIM2, NUM_CLASSES, DROPOUT, LAMBDA, ALPHA)
...
def main():
    ...
    aver_epoch = cross_validation(train_dataframe, fold_number=5)      #train_5CV
